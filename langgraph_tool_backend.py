...
def search_tool(query: str) -> str:
    """
    Search DuckDuckGo for web search results about any topic, question, or real-time info.
    """
    try:
...
def chat_node(state:chatState):
    ...
    system_prompt = SystemMessage(content=(
        "You are an advanced AI assistant. Use tools when necessary to resolve user requests. "
        "IMPORTANT: When invoking a tool, use the native tool calling schema. "
        "Do NOT write XML-like tags such as '<function=...>' or raw text descriptions of function calls."
    ))
    res = llm_with_tool.invoke([system_prompt] + message)
    return {'messages' : [res]}
